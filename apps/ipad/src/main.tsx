...
const totalKept = (clip: Clip) => clip.segments.reduce((sum, segment) => sum + Math.max(0, segment.end - segment.start), 0);
const normalizeSegments = (segments: Segment[], duration: number) => segments.map((segment) => ({ start: clamp(segment.start, 0, duration), end: clamp(segment.end, 0, duration) })).filter((segment) => segment.end - segment.start >= 0.05).sort((a, b) => a.start - b.start);
...
function parseCommand(command: string, clips: Clip[], selectedId: string | null, vertical: boolean): CommandResult {
  ...
  if (/\b(reset|clear|undo all)\b/.test(text)) nextClips = nextClips.map((clip) => ({ ...clip, segments: [{ start: 0, end: clip.duration }] }));
  ...
  if (selected && targetIndex >= 0 && targetIndex < nextClips.length) {
    ...
    else if (middle) { const amount = Number(middle[1]); const center = edited.duration / 2; edited = removeRange(edited, center - amount / 2, center + amount / 2); }
    else if (keep) { const start = clamp(Number(keep[1]), 0, edited.duration); const end = clamp(Number(keep[2]), start + 0.05, edited.duration); edited = { ...edited, segments: [{ start, end }] }; }
...
function formatSrtTime(seconds: number) { const ms = Math.max(0, Math.round(seconds * 1000)); const h = Math.floor(ms / 3600000); const m = Math.floor((ms % 3600000) / 60000); const s = Math.floor((ms % 60000) / 1000); const milli = ms % 1000; return `${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}:${String(s).padStart(2, "0")},${String(milli).padStart(3, "0")}`; }
...
function App() {
  ...
  const [captionStart, setCaptionStart] = React.useState(0);
  const [captionEnd, setCaptionEnd] = React.useState(3);
  ...
  React.useEffect(() => { setPlan({ label: vertical ? "9:16 vertical" : "Original format", vertical }); }, [vertical]);
  ...
  const applyCommand = () => {
    ...
    if (phase6.handled) {
      setClips(phase6.clips as Clip[]); setCaptions(phase6.captions as Caption[]); setSelectedId(result.selectedId); setVertical(result.vertical); setStatus(phase6.message); setCommand(""); return;
    }
    setClips(result.clips); setSelectedId(result.selectedId); setVertical(result.vertical); setStatus(result.message); setCommand("");
  };
  ...
  const exportCaptions = () => {
    if (!captions.length) { setStatus("Add at least one caption first"); return; }
    const blob = new Blob([captionsToSrt(captions)], { type: "application/x-subrip;charset=utf-8" });
    const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "ai-video-editor-captions.srt"; a.click(); URL.revokeObjectURL(url);
    setStatus(`Caption track exported • ${captions.length} subtitle${captions.length === 1 ? "" : "s"} • local-only`);
  };

  const addCaption = () => {
    const text = captionText.trim(); if (!text || !selectedClip) return;
    const start = clamp(captionStart, 0, Math.max(0, selectedClip.duration - 0.05));
    const end = clamp(captionEnd, start + 0.05, selectedClip.duration);
    setCaptions((current) => [...current, { id: makeId(), clipId: selectedClip.id, start, end, text }].sort((a, b) => a.start - b.start));
    setCaptionText(""); setStatus("Caption added locally");
  };
  ...
  const moveClip = (index: number, direction: -1 | 1) => {
    const target = index + direction; if (target < 0 || target >= clips.length) return;
    const next = [...clips]; [next[index], next[target]] = [next[target], next[index]]; setClips(next); setStatus(`Moved clip ${index + 1} ${direction < 0 ? "up" : "down"}`);
  };

  const deleteClip = (index: number) => {
    const removed = clips[index]; const next = clips.filter((_, i) => i !== index); setClips(next);
    if (removed?.id === selectedId) setSelectedId(next[Math.min(index, next.length - 1)]?.id || null);
    setCaptions((current) => current.filter((caption) => caption.clipId !== removed?.id)); setStatus(`Deleted clip ${index + 1}`);
  };

  const updateSelectedRange = (start: number, end: number) => {
    if (!selectedClip) return;
    const safeStart = clamp(start, 0, Math.max(0, end - 0.1)); const safeEnd = clamp(end, safeStart + 0.1, selectedClip.duration);
